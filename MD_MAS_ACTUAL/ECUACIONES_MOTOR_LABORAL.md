...
## **0.1 Cambios Normativos Post-Ley 27.802 (Marzo 2026)**

**DEROGACIONES — Efectivas desde Ley 27.802 (Marzo 2026)**:
- ❌ **Ley 24.013 Arts. 8/9/10** — Sanciones por falta de registro (DEROGADOS)
- ❌ **Ley 25.323 Arts. 1/2** — Duplicación indemnización (DEROGADA)
- ✅ **Art. 80 LCT** — Certificados deficientes (MANTIENE VIGENCIA)

**NUEVOS ANÁLISIS — Ley 27.802**:
- ✅ **Art. 23 LCT** — Presunción laboral condicionada (facturación + pago bancario)
- ✅ **Art. 30 LCT** — Responsabilidad solidaria (exención con 5 controles)

**Regla de Transición Temporal**:

$$M_{aplicable} = \begin{cases}
M_{24013 / 25323} & \text{si } fecha < \text{01/03/2026 (hechos previos a reforma)} \\
0 & \text{si } fecha \geq \text{01/03/2026 (post-reforma, multas NO aplican)} \\
\text{ALERTA} & \text{si } fecha > \text{09/07/2024 AND fecha < \text{01/03/2026 (zona gris)}}
\end{cases}$$

**Interpretación**:
- **Hechos anteriores a 01/03/2026**: Puede aplicarse régimen anterior (litigios en transición)
- **Hechos posteriores a 01/03/2026**: Leyes 24.013 y 25.323 NO aplican (modelo nuevo: fraude + daños)
- **Sistema ahora genera**: Alertas de fraude laboral + daños complementarios en lugar de multas

---

## **0.2 Evaluación de Presunción Laboral (Art. 23 LCT)**

**La presunción de laboralidad NO OPERA cuando existe**:

$$P_{laboral} = \begin{cases}
\text{APLICA} & \text{si } facturacion = 0 \text{ Y } pago\_bancario = 0 \\
\text{NO APLICA} & \text{si } facturacion = 1 \text{ Y } pago\_bancario = 1 \\
\text{PARCIAL} & \text{otros casos}
\end{cases}$$

**Impacto en complejidad probatoria**:
- Si presunción NO aplica: Complejidad aumenta -0.5 en IRIL (requiere prueba reforzada)
- Favorece defensa del empleador (requiere activa demostración de dependencia)

---

## **0.3 Evaluación de Responsabilidad Solidaria (Art. 30 LCT)**

**Principal EXENTO de solidaria si valida 5 controles**:

$$R_{solidaria} = \begin{cases}
R_{máximo} & \text{si controles\_validados} = 0 \\
0 & \text{si controles\_validados} = 5 \\
R_{escala} & \text{si controles\_validados} \in (1,4)
\end{cases}$$

**Los 5 controles**:
1. CUIL de trabajadores (verificable AFIP)
2. Aportes seguridad social al día (verificable AFIP)
3. Pago directo de salarios (cuenta bancaria)
4. CBU del trabajador (bancarizado)
5. Cobertura ART endosada a principal

---

## **0.4 Tabla de Validación Automática**

El sistema debe generar una **alerta normativa**:

```
✓ Fecha de extinción: 15/06/2024 → Zona pre-reforma (Art 23/30 no aplican)
✗ Fecha de extinción: 25/08/2024 → Zona transición (9/7/24 < fecha < 1/3/26)
! Fecha de extinción: 14/04/2026 → POST-REFORMA: Aplican Art. 23, 30, fraude + daños
```
